...
function ScrollPicZHHY(count, id, url, type, width,height, callback) {
    this.url = url;
    this.id = id;
    var _this = this;
    this.speed = 15;
    this.MyMar = null;
    this.MarqueeAction = 0;
    this.MarqueeSpeed = 10;
    this.count = count; //每版多少个
    if (!width) { width = 880; }
    this.width = width;
    var page = 1;//当前第几页
    var allCount = 0;//所有的图片数
    var allPage = 1; // 总共多少页
    this.isMoving = false;//指示是否正在移动
    var viewWnd, viewlist, leftBtn, rightBtn;
    if (!height) {
        height = 190;
    }
    ...
    htmlHead += '</td>';
    htmlHead += '<td style="padding: 5px 0 5px 0">';
    htmlHead += '<div id="viewWnd" style="overflow: hidden; height: '+height+'px; width:' + width + 'px; margin-left: 0px;margin-right: 0px;">';
    htmlHead += '<table width="cellspacing=0">';
    htmlHead += '<tr>';
    htmlHead += '<td id="demo4" class="demo">';
    htmlHead += '<table width="100%" style="text-align: center">';
    ...
    this.Marquee = function (action, speed) {
        if (action == 0) {//向左移动
            if (page == 1) {
                setBtnClass();
                return false;
                ...
                page--;
                setBtnClass();
                _this.move(viewWnd.scrollLeft - viewWnd.offsetWidth, speed);
            }
        } else {//右移
            if (page == allPage) {
                setBtnClass();
                ...
                page++;
                setBtnClass();
                _this.move(viewlist.offsetWidth - viewWnd.offsetWidth, speed);
            } else {
                page++;
                setBtnClass();
                _this.move(viewWnd.scrollLeft + viewWnd.offsetWidth, speed);
    ...
    this.bindEvent = function () {
        ...
        leftBtn.click(function () {
            if (!_this.isMoving) {
                _this.Marquee(0, _this.speed);
            }
        });
        rightBtn.click(function () {
            if (!_this.isMoving) {
                _this.Marquee(1, _this.speed);
            }
        });
    }
    ...
    this.move = function (pos, speed) {
        _this.isMoving = true;
        var curPos = viewWnd.scrollLeft;
        if (curPos == pos) {
            _this.isMoving = false;
            return false;
        } else if (curPos < pos) {
            if (curPos + _this.MarqueeSpeed > pos) {
                viewWnd.scrollLeft = pos;
            } else {
                viewWnd.scrollLeft += _this.MarqueeSpeed;
            }
        } else {
            if (curPos - _this.MarqueeSpeed < pos) {
                viewWnd.scrollLeft = pos;
        ...
        setTimeout(function () {
            _this.move(pos, speed);
        }, speed);
    };

    this.load = function () {
        _this.tDiv = $("#" + _this.id + " table td:eq(1)").find("#viewWnd");
        viewWnd = _this.tDiv[0];
        _this.tDivTd = $("#" + _this.id + " table td:eq(1)").find("div table td.demo");
        viewlist=_this.tDivTd[0];
        _this.tDivTd2 = $("#" + _this.id + " table td:eq(1)").find("div table td.demoCopy");
        leftBtn = $("#" + _this.id + " .scr_left");
        rightBtn = $("#" + _this.id + " .scr_right");
        _this.bindEvent();
    }
