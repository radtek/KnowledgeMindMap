...
function CustomDeleteItem(obj, func, parameter, parameter1) {
    CustomDeleteItemToolTip("确定要删除该对象吗？删除后将无法恢复", obj, func, parameter, parameter1);
}

//通用删除方法(对象)
function CustomDeleteItemToolTip(toolTip,obj, func, parameter, parameter1) {
    var deaultToolTip = "确定要删除该对象吗？删除后将无法恢复";
    if (toolTip) {
        deaultToolTip = toolTip;
    }
    var a = confirm(deaultToolTip);
    if (!a) return;
    var tbName = ""
