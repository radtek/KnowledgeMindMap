...
MenuArr[0] = [{ "name": "工作平台", "url": "/DesignManage/NewHome_ZHHY" },
    ...
    { "name": "设计变更管理", "url": "" },
    { "name": "设计师之家", "url": "/DesignerHome/QuestionIndex" },
...
//    { "name": "项目指标情况汇总表", "url": "/ProjectManage/IndexStatistics" },
//    { "name": "限额设计控制情况汇总表", "url": "/ProjectManage/DesignIndexStatistics" }
...
MenuArr[6] = [
    { "name": "设计单位库", "url": "/Supplier/Designsupplier" }
    ];

MenuArr[7] = [{ "name": "通知通告", "url": "/DesignManage/DesignKnowledge?libId=1" },
    { "name": "制度流程", "url": "/DesignManage/DesignKnowledge?libId=2" },
    { "name": "标准模块", "url": "/DesignManage/DesignKnowledge?libId=3" }
    ];
//    { "name": "苗木库", "url": "/Material/MaterialSeedlings"}

MenuArr[9] = [
    { "name": "设计师园地", "url": "/DesignerHome/QuestionIndex" }
//    { "name": "新同事风采", "url": "/DesignerHome/NewEmpIndex" }
];
//    { "name": "苗木库", "url": "/Material/MaterialSeedlings"}
...
//    { "name": "在线模板", "url": "/DesignManage/BookTaskTemplates" },
    ...
    { "name": "首页管理", "url": "/DesignManage/HomeIndexManage" }
//    {"name":"指标管理","url":"/ProjectManage/IndexTemplateManage"}
    ];
    ...
    if (MenuArr[index]) {
        ...
        for (var x = 0; x < MenuArr[index].length; x++) {
            ...
            if (MenuArr[index][x].url != "" && hasMenuRight == true) {
                if (x == 14) {
                    html += "<li><a class='menuopen' hidefocus='true' href='javascript:;' onclick='showAllMenu(this)'></a></li>"
                    html += "<li style='display:none;'  mark='hide'><a hidefocus='true' href='" + MenuArr[index][x].url + "'>" + nameArr + "</a></li>";
...
function showAllMenu(obj) {
    if ($(".nav_lev2").find("li[mark]:hidden").length>0) {
        $(".nav_lev2").find("li[mark]:hidden").show();
        $(obj).attr("class","menuclose");
    } else {
        $(".nav_lev2").find("li[mark=hide]").hide();
        $(obj).attr("class", "menuopen");
    }
}