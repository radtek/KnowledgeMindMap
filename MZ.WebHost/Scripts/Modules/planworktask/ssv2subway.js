...
var subWayMap = (function () {
    ...
    function __setSubWay() {
        ...
        var color = color = { "1": "#b5b5b5", "2": "#ffcc33", "3": "#66cc40", "4": "#fb5839" }, swid, swname, doc = YH.isStrict ? document.documentElement : document.body;
...
function getRelTasks(planId, sid, obj, pointId){
	...
	$.ajax({
	    type: 'POST',
	    url: '/Home/DecisionNewPointTask/' + planId + '?textPid=' + sid + "&showMutilPlanTask=True", cache: false,
	    success: function (rs) {
	        var i = 0, cc = ['<table cellpadding="0" cellspacing="0">'], has = false, _;
	        for (; i < rs.length; ++i) {
	             _ = rs[i];
	             cc.push('<tr><td height=25 class="fontblue"></td><td class="fontblue" width="250"><a target="_blank" href="/DesignManage/ProjTaskInfo/' + _.id + '?retType=1">' + _.name + '</a></td><td width="100">' + _.otherParam + '</td><td class="fontorange" width="40">' + _.value + '</td></tr>'); has = true;
	        }
	        cc.push("</table>");
	        if (has) {
	            tpane.html(cc.join(""));
