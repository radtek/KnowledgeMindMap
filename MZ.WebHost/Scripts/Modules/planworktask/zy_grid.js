...
var commApi = {
   ...
   // "saveTask": '/Home/SaveTaskInfo', // 保存任务
    ...
    'imgUrl': __globalDomain + '/Content/images/client/ZY/', // 任务状态图标目录
    'saveBgColor': '/Home/ChangeDateBgColor/', // 保存日期单元格底色
    'taskStatImg': { '2': 'flag_7.png', '3': 'flag_7.png', '4': 'flag_3.png', '-1': 'flag_2.png' }, // 任务状态图标
    "tpl": ['<tr id="tr{taskId}" cp="1" isEdit="{isEdit}" noDelete="{noDelete}" nodeLevel="{nodeLevel}">',
		'<td field="torder" class="datagrid-td-rownumber"><div class="datagrid-cell-rownumber">{rowindex}</div></td>',
		'<td field="name" class="{cls}" width="250"><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" id="img{taskId}" src="' + __globalDomain + '/Content/images/DesignManage/ico14.jpg"></div><div style="float:left;width:{nwidth}px">{name}</div></td>',
        //'<td field="levelId" style="text-align:center" width="88">{levelName}</td>',
        '<td field="period" style="text-align:center" width="40">{period}</td>',
        '<td field="startDate"{clrs} style="text-align:center" width="120">{startDate}</td>',
		'<td field="endDate"{clre} style="text-align:center" width="120">{endDate}</td>',
		'<td field="ownerName" width="70">{ownerName}</td>',
        '<td field="nodeOrgId" width="70">{nodeOrgName}</td>',
        '<td field="nodeProfId" width="95">{nodeProfName}</td>',
        '<td field="nodeTypeId" width="95">{nodeName}</td>',
        '<td field="hasApproval" style="text-align:center" width="60">{SetApproval}</td>',
        '<td field="projStage" width="110">{stageName}</td>',
        '<td field="pointName" swid="{strPointId}" width="150">{pointName}</td>',
        '<td field="relConDiagName" cdid="{relConDiagId}" width="110">{relConDiagName}</td>',
		'<td field="status" style="text-align:center" class="norbor" width="40">{statusName}</td>',
		'</tr>'],
...
var hoverbg = '#d0e5f5', selbg = '#fcf1a9', copyDate = { planId: 0, taskId: 0, field: '', v: '', f: '' },
 ...
 ///检查等级权限
function checkLevelRight(level) {
   
    switch (level) {
        case 1:
            if (typeof (planLevel1) == "undefined" || planLevel1 == "True") {
                return true;
            }
             break;
         case 2:
             if (typeof (planLevel2) == "undefined" || planLevel2 == "True") {
                 return true;
             } break;
         case 3:
             if (typeof (planLevel3) == "undefined" || planLevel3 == "True") {
                 return true;
             }
             break;
        default:
...
function a2DataGrid(config) {
    ...
    this.init = function () {
		...
		.dblclick(function () {
		    ...
		    //检查权限
		    var curField = cell.td.getAttribute("field");
		    if (curField == "levelId") {
		        if (typeof (planLevelChange) != undefined && planLevelChange == "False") {
		            return false;
		        }
		    } else {
		        if (!checkLevelRight(self.getTaskFieldData(tid, 'levelId'))) {
		            return false;
		        }
		    }
		    if (!self.hasRight) {
        ...
        this.grid.find("input.imexp2").click(function () {
            self.importExp2();
        });
    ...
    this.showColorpk = function () {
        ...
        //检查权限
        if (!checkLevelRight(this.getTaskFieldData(tid, 'levelId'))) {
            return false;
        }
        if (!this.hasRight) {
...
a2DataGrid.prototype.loadData = function () { // 加载数据
    ...
    $(".loadmask").css({ "width": "100%", "height": "100%" });
    var queryData={ planId: self.secdPlanId };
    var isShowAll = false;
    if ($("#p-j-showAllTask").length == 1 && $("#p-j-showAllTask").is(":checked")) { 
        queryData.isShowAll=1;
    }
    gLoadQueue.add({
        dataType: 'JSON', cache: false,
        url: commApi.getPlanData, data: queryData,
        success: function (rs) {
            var r = rs;
...
a2DataGrid.prototype.calcrs_ = function (dt, lvl) {
    ...
    if (dt['name'] != "请填写任务名称") {
        ...
        _url = _isexpLibId ? '<a href="javascript:;">' : '<a href="' + commApi.taskUrl + _tid + '?isEdit=' + isEdit + '" target="_blank" class="' + _cls + '">';
    ...
    dt["relConDiagName"] = String.cut(dt["relConDiagName"] || "", 12, '...');
    dt['clrs'] = clrs; dt['clre'] = clre;
    if (dt.status <= 2 && dt.status > -1) {
        dt["statusName"] = '<img src="' + commApi.imgUrl + commApi.taskStatImg[2] + '" alt="未开始" />';
    } else if (dt.status != 5) {
...
a2DataGrid.prototype.addRemark = function () { // 添加备注
    var tpl = '<textarea style="width:380px;height:140px; class="textarea_01"></textarea>', self = this, tid = self.curCell.tid,
...
a2DataGrid.prototype.initEv = function () {
    ...
    this.dt.find("tr[isEdit=true]").unbind(".wkgrid").bind("mousedown.wkgrid", function (ev) {
...
a2DataGrid.prototype.initDrag_ = function () {
    ...
    var moveid, tomoveid, movetype, self = this, trs = this.dt.find("tr[isEdit=true]"),
...
a2DataGrid.prototype.menu = function (e) { // menu
    ...
    _menu.find(".menu-jfw").unbind("click").bind("click", function () {
        ...
        if (typeof boxJfw != 'undefined' && $.isFunction(boxJfw)) {
            boxJfw(_ids);
        }
        _menu.hide();
    });
    _menu.find(".menu-setJfw").unbind("click").bind("click", function () {
        var _ids = '';
        for (var tid in self.selRows) {
            _ids = tid;
            break;
    ...
    _menu.find(".menu-setLink").unbind("click").bind("click", function (ev) {
        setTaskLink(cell.tid); _menu.hide();
    })
    _menu.find(".menu-cdNode").unbind("click").bind("click", function (ev) {
        setContextDiagram(cell.tid); _menu.hide();
    });
    _menu.find(".menu-setForm").unbind("click").bind("click", function (ev) {
        setOldRelation(cell.tid); _menu.hide();
    })
    ...
    _menu.find(".menu-delTask").each(function () {
        if ($("#tr" + cell.tid).attr("noDelete") == "true") {
    ...
    _menu.find(".menu-taskDes").unbind("click").bind("click", function (ev) {
        setTaskDes(cell.tid);
    });
    ...
    _menu.find(".menu-appro").unbind("click").bind("click", function () {
        apprSetJH.init.call(self, cell.tid); _menu.hide();
    });
    _menu.find(".menu-lchm").unbind("click").bind("click", function () {
        self.pexe.call(self, true); _menu.hide();
    });

    _menu.find(".menu-homeindex").unbind("click").bind("click", function () {
        self.homeIndexSetting.call(self, true); _menu.hide();
    });
...
a2DataGrid.prototype.homeIndexSetting = function () { // 设置首页任务推送
    var tpl = $('<div style="width:360px;height:130px;"><ul style="display:block;margin:8px"></ul></div>'),
    ...
    $.YH.box({
        ...
        ok: function () {
            var $box = $(this), arr = {};
            if ($("input[name=isShowIndex]", $box).is(":checked")) {
                arr.isShowIndex = 1;
            } else {
                arr.isShowIndex = 0;
            }
            arr.projStageId = $("select[name=meetingOption]", $box).val();
            self.saveTask(tid, arr);
        }
    });

    function getISI(o) {
        if (commApi.projlinkData == undefined) {
            alert('未设置commApi.projlinkData');
            return false;
        }
        ...
        $.post(commApi.projlinkData,
        {
            ...
            $(data.taskInfoList).each(function (i, item) {
                //isLink 联动节点
                if (item.isShowIndex == "1") { itc = "checked=checked"; } else { itc = ""; } //阶段性成果Id
                htmlLi = '<li class="block p5"><strong>' + item.name + '</strong>：<input type="checkbox" ' + itc + ' name="isShowIndex" >设置为首页提醒任务</li>';
                if (data.meetingOption != undefined) {
                    var tempSelect = '<li class="block p5">所属会议节点：<select name="meetingOption" >';
                    tempSelect += '<option value="">无</option>';
                    $(data.meetingOption).each(function (i, item2) {
                        tempSelect += '<option ';
                        if (item.projStageId == item2.value) {
                            tempSelect += ' selected ';
                        }
                        tempSelect += 'value="'
                        tempSelect += item2.value;
                        tempSelect += '">';
                        tempSelect += item2.name;
                        tempSelect += '</option>';
                    });
                    tempSelect += '</select></li>';
                    htmlLi += tempSelect;
                }
            });
            o.find("ul").html(htmlLi);
            //及时不推送首页应该也可以设置会议节点
            //            o.find("ul input[name=isShowIndex]").click(function () {
            //                var orderObj = o.find("select[name=homeOrder]").parent();
            //                $(this).is(":checked") ? orderObj.show() : orderObj.hide();
...
function lvlFormatter(value) {
    ...
    for (; i < xhLevel.length; i++) {
        if (value == xhLevel[i].lvlid) isck = true;
        str += '<option value="' + xhLevel[i].lvlid + '"' + (isck ? " selected" : "") + '>' + xhLevel[i].lvlname + '</option>';
        isck = false;
    }
    return str;
}
...
function nodeFormatter(value) {
    var str = "", i = 0, isck = false;
    if (taskNode && taskNode instanceof Array) {
        for (; i < taskNode.length; i++) {
            if (value == taskNode[i].nodeid) isck = true;
            str += '<option value="' + taskNode[i].nodeid + '"' + (isck ? " selected" : "") + '>' + taskNode[i].nodename + '</option>';
            isck = false;
        }
    }
    return str;
...
//    var str = "", i = 0, isck = false;
//    if (taskOrg && taskOrg instanceof Array) {
//        for (; i < taskOrg.length; i++) {
//            if (value == taskOrg[i].orgid) isck = true;
//            str += '<option value="' + taskOrg[i].orgid + '"' + (isck ? " selected" : "") + '>' + taskOrg[i].orgname + '</option>';
//            isck = false;
//        }
    //    }
    var str = "", isck = false;
    $.ajax({
        ...
        success: function (data) {
            $(data).each(function (i,n) {
                if (value == data[i].taskOrgId) isck = true;
                str += '<option value="' + data[i].taskOrgId + '"' + (isck ? " selected" : "") + '>' + data[i].name + '</option>';
                isck = false;
            })
...
function profFormatter(value) {
//    var str = "", i = 0, isck = false;
//    if (taskProf && taskProf instanceof Array) {
//        for (; i < taskProf.length; i++) {
//            if (value == taskProf[i].profid) isck = true;
//            str += '<option value="' + taskProf[i].profid + '"' + (isck ? " selected" : "") + '>' + taskProf[i].profname + '</option>';
//            isck = false;
//        }
    //    }
    var str = "", isck = false;
    $.ajax({
        ...
        success: function (data) {
            $(data).each(function (i,n) {
                if (value == data[i].profId) isck = true;
                str += '<option value="' + data[i].profId + '"' + (isck ? " selected" : "") + '>' + data[i].name + '</option>';
                isck = false;
            })
        }
    });
...
function projStageFormatter(value) {
    var str = "", i = 0, isck = false;
    if (typeof projStageArr != "undefined") {
        for (; i < projStageArr.length; i++) {
            if (value == projStageArr[i].stageId) isck = true;
            str += '<option value="' + projStageArr[i].stageId + '"' + (isck ? " selected" : "") + '>' + projStageArr[i].name + '</option>';
            isck = false;
        }
    }
    return str;
...
a2DataGrid.prototype.showEditor = function (l, t, w, h) { // 编辑单元格
    ...
    if (_col.editor) {
        ...
        if (_col.field == 'name' && relConDiagId > 0 && isTemplate != 1) {
        ...
        if (!self.setRelations) {
            ...
            inputfiled = $("<input/>").addClass("inputField").css({ width: w - 2, height: 22 + 'px', 'line-height': 22 + 'px', 'padding': '0' }).val(_txt);
        ...
        switch (_editor) {
            ...
            case "singleLineText":
                ...
                if (_col.field == 'ownerName') {
                    var up = $('<input type="hidden" class="data-uid" /><input type="hidden" class="data-profid" /><input type="hidden" class="data-username" />'), _o1, _o2;
                    this.inputDiv.append(up);
                    _o1 = this.inputDiv.find("input.data-uid"); _o2 = this.inputDiv.find("input.data-profid");
                    var _o3 = this.inputDiv.find("input.data-username");
            ...
            case "dummy":
                if (_col.field == "taskRelations") { // 前置任务
                    ...
                    if ($("#wrapplan" + self.secdPlanId).attr('mapId') != undefined && $("#wrapplan" + self.secdPlanId).attr('mapId') != "0") {
                        subWayMap.viewPoint({ planId: self.secdPlanId,
                            modal: true, swid: swid, nobutton: true, type: 'set',
                            callback: function (swid, t) {
                                if (swid == "-1") {
                                } else {
                                    var ck = self.checkModified(cell.tid, _col.field, swid);
                                    if (ck) {
                                        self.updateFieldData(cell.tid, 'strPointId', swid);
...
a2DataGrid.prototype.checkField = function () { // 保存前检查
    ...
    if (flag) {
        if (_editor == "Date") {
            ...
            result = _select.text();
            if (_col.field == 'levelId' || _col.field == 'nodeTypeId' || _col.field == 'nodeOrgId' || _col.field == 'nodeProfId' || _col.field=="projStage") {
...
a2DataGrid.prototype.saveData = function (data) { // 保存 任务数据
    ...
    if (data.modified) {
        ...
        if (data.type == 'formula') { // 进入公式保存处理
            ...
            // 任务数据
            var dt = data.result;
            if (data.field == 'levelId' || data.field == 'nodeTypeId' || data.field == 'nodeOrgId' || data.field == 'nodeProfId' || data.field == "projStage") {
                dt = data.data.plvl;
...
a2DataGrid.prototype.saveTask = function (taskId, options) { // ajax task
    ...
    postDt['needSplit'] = "0"; postDt['splitUserId'] = "-1"; postDt['splitProfId'] = "0";
    postDt['levelId'] = Dt.levelId; 
    postDt['nodeTypeId'] = Dt.nodeTypeId;
    postDt['nodeOrgId'] = Dt.nodeOrgId;
    postDt['nodeProfId'] = Dt.nodeProfId;
    postDt['projStageId'] = Dt.projStage;
...
a2DataGrid.prototype.calcFormula = function (formula, fRef, expression) { // S37 [S32,G32] S32+G32-5
    ...
    for (; i < fRef.length; ++i) {
        ...
        fRefData = String(this.getTaskFieldData(rr[2], ccodeToField[rr[1].toUpperCase()])).replace("-", "/");
...
var ckFunction = {
    ...
    "dateDiff": function (record) {//计算工期
        var s = record['startDate'], e = record['endDate'], p = record['period'], diff;
        if (s == '' || e == '') return false;
        s = new Date(s && s.replace(/-/g, "/"));
        e = new Date(e && e.replace(/-/g, "/")); p = parseInt(p, 10) || 0;
        diff = parseInt(Math.abs(Date.parse(s) - Date.parse(e)) / 1000 / 60 / 60 / 24, 10) + 1;
...
a2DataGrid.prototype.importExp = function () { // 导入经验
    var expData = [], _secdPlanId = -1, self = this, secdPlanId = self.secdPlanId, setoffset_year, setoffset_month, setoffset_day, isIgnorePerson, isIgnoreDate, isIgnorePassed;
    $.YH.box({
        target: commApi.importExp + secdPlanId,
        title: '载入公司经验',
        width: 900,
        modal: true,
        destroy: true,
        afterLoaded: function () {
            var o = $(this);
            var _o1 = o.find("#manager"), _o2 = o.find("#managerId");
            bindASearchableInput(_o1, _o2, null, secdPlanId, 2, false, false);
            o.find("li[secdPlanId]").each(function () {
                $(this).click(function () {
                    _secdPlanId = $(this).attr("secdPlanId");
                    o.find("li[secdPlanId]").removeClass("libg");
                    $(this).addClass("libg");
                    o.mask("loading...");
                    $(".loadmask").css({ "width": "100%", "height": "100%" });
                    $.ajax({
            ...
            $("#searchexp").click(function () {
                var compname = $.trim(o.find("input[name='companyname']").val()),
					compexp = $.trim(o.find("input[name='companyexp']").val()), lastobj;
                o.find(".cnclor").removeClass("cnclor");
                o.find(".expbg").removeClass("expbg");
                compname && o.find("div[data-orgname='1']").each(function () {
                ...
                !compname && compexp && o.find("li[data-expname='1']").each(function () {
                    if ($(this).text().toLowerCase().indexOf(compexp) != -1) {
                        $(this).addClass("expbg");
                        lastobj = this;
                    }
            ...
            setoffset_year = o.find("input[name='OffSetYear']");
            setoffset_month = o.find("input[name='OffSetMonth']");
            setoffset_day = o.find("input[name='OffSetDay']");
            isIgnorePerson = o.find("input[name='isIgnorePerson']");
            isIgnoreDate = o.find("input[name='isIgnoreDate']");
            isIgnorePassed = o.find("input[name='isIgnorePassed']");
            setoffset_year.onumberbox(); setoffset_month.onumberbox(); setoffset_day.onumberbox();
            o.find("li[secdPlanId]").first().click();
        },
        ok: function () {
            if (_secdPlanId == -1) {
            ...
            if (confirm("载入模板将删除已有数据及其数据,确定载入吗？")) {
                $("#container").mask("载入中...");
                $(".loadmask").css({ "width": "100%", "height": "100%" });
                ...
                $.ajax({
                    url: commApi.iExpSave + _str + "&isNoUpdatePlan=1",
                    data:
                    {
                        copySecPlanId: _secdPlanId,
                        sourceSecPlanId: self.secdPlanId
                    },
                    type: 'POST',
                    //complete: function () { $this.unmask(); },
                    success: function (data) {
                        $("#container").unmask();
                        if (data.Success) {
...
a2DataGrid.prototype.importExp2 = function () { // 导入经验
    var expData = [], _secdPlanId = -1, self = this, secdPlanId = self.secdPlanId, setoffset_year, setoffset_month, setoffset_day, isIgnorePerson, isIgnoreDate, isIgnorePassed;
    $.YH.box({
        target: commApi.importExp + secdPlanId,
        title: '载入公司经验',
        ...
        ok: function () {
            ...
            if (confirm("载入模板将删除已有数据及其数据,确定载入吗？")) {
                ...
                $.ajax({
                    url: commApi.iExpSave2 + _str + "&isNoUpdatePlan=1",
                    data:
                    {
                        copySecPlanId: _secdPlanId,
                        sourceSecPlanId: self.secdPlanId
...
a2DataGrid.prototype.preTask = function () { // 费用支付选择基本计划中的节点作为前置节点
    ...
    $.YH.box({
        ...
        open: function () {
            var o = $(this);
            o.find(".nodeList").load("/DesignManage/ProjTaskSelectNode?taskId=" + tid + "&nodeTypeId=0&relationType=0&r=" + Math.random());
            o.find(".nodeSearch").find("a[s]").each(function () {

                $(this).click(function () {
                    $(this).addClass("select").siblings().removeClass("select");
                    if ($(this).attr("s") == "s") {
                        o.find(".nodeList").load("/DesignManage/ProjTaskSelectNode?taskId=" + tid + "&nodeTypeId=0&relationType=0&r=" + Math.random(), function () {

                        });
                    } else {
                        o.find(".nodeList").load("/DesignManage/ProjTaskSelectNode?taskId=" + tid + "&nodeTypeId=2&relationType=0&r=" + Math.random(), function () {
            ...
            o.find(".nodeSearch").find("a[t]").each(function () {

                $(this).click(function () {
                    if ($(this).attr("t") == "t") {
                        var keyword = $(this).parent().find("input").val();
                        o.find(".nodeList").load("/DesignManage/ProjTaskSelectNode?taskId=" + tid
                                + "&nodeTypeId=0&relationType=0&keyWord=" + escape(keyword) + "&r=" + Math.random(), function () {

                                });
                    } else {

                        if (o.find("a.select").attr("s") == "s") {
                            nodeTypeId = 0;
                            relationType = 0;
                        } else {
                            nodeTypeId = 2;
                        ...
                        o.find(".nodeList").load("/DesignManage/ProjTaskSelectNode?taskId=" + tid + "&nodeTypeId=" + nodeTypeId + "&relationType=" + relationType + "&r=" + Math.random(), function () {
        ...
        ok: function () {
            ...
            var obj = o.find("form");
            var keys = "";
            $("input[name=key]:checked", o).each(function () {
                var isTimeAlert = $(this).closest("tr").find("input[name=isTimeAlert]").is(":checked");
                if (isTimeAlert != undefined) {
                    if (isTimeAlert) {
                        keys += $(this).val() + ",1" + "|Y|";
                    }
                    else {
    ...
    function saveTaskRelationInfo(obj) {
        ...
        $.ajax({
            url: "/Home/QuickSaveTaskRelationExtend",
            type: 'post',
            data: formdata,
            dataType: 'json',
            error: function () {
...
var apprSetJH = (function () {
    var taskId;
    function showPanel(t) {
        $.YH.box({
            target: '/DesignManage/ProjTaskFlow?taskId=' + taskId + '&r=' + Math.random(),
            title: '审批流程设置',
            width: 700,
            afterLoaded: function () {
                changeFlowInfoDiv();
