...
var commApi = {
    ...
    "saveTask": '/Home/SaveTaskInfoZHHY', // 保存任务
    ...
    "batchFzr": '/Home/BatchSetTaskOwnerZHHY/', // 批量设置负责人
    ...
    "tpl": ['<tr id="tr{taskId}" cp="1">',
		...
		'<td field="name" class="{cls}" width="379"><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" id="img{taskId}" src="' + __globalDomain + '/Content/images/DesignManage/ico14.jpg"></div><div style="float:left;width:{nwidth}px">{name}</div></td>',
        '<td field="period" style="text-align:center">{period}</td>',
        '<td field="startDate"{clrs} class="dotorange">{startDate}</td>',
		'<td field="endDate"{clre}  class="dotorange">{endDate}</td>',
...
a2DataGrid.prototype.setISI = function () { // 是否首页显示
    ...
    box(tpl, { boxid: 'ISI', title: '任务设置显示(打钩表示设置成功)', cls: 'shadow-container', modal: true, width: 400,
        ...
        submit_cb: function (o) {
            ...
            if ($("input[name=isIndicator]", o.db).attr("checked") == "checked") {
                lrIndictor = 1;
            } else {
                lrIndictor = 0;
            }
            if ($("input[name=isMaterial]", o.db).attr("checked") == "checked") {
                lrMaterial = 1;
            } else {
                lrMaterial = 0;
            }
            if ($("input[name=isResult]", o.db).attr("checked") == "checked") {
                lrResult = 1;
            } else {
                lrResult = 0;
            }
            ...
            arr.indicatorRemark = $("input[name=indicatorRemark]", o.db).val();
            arr.materialRemark = $("input[name=materialRemark]", o.db).val();
            arr.resultRemark = $("select[name=resultRemark]", o.db).val();
            if (lrId == 1) {
                arr.homeOrder = $("input[name=homeOrder]", o.db).val();
            }
            else { arr.homeOrder = ""; }
...
a2DataGrid.prototype.batchFuze = function (ev) { // 批量设置负责人
    ...
    getTaskPeopleListForOneClick(x, y, projId, 2, function (_userId, _profId, _userName, userArr) {
        $.ajax({
...
a2DataGrid.prototype.importExp = function () { // 导入经验
    var expData = [], _secdPlanId = -1, self = this, secdPlanId = self.secdPlanId, setoffset_year, setoffset_month, setoffset_day, isIgnorePerson, isIgnoreDate, isIgnorePassed, insertPlanName;

    box(commApi.importExp + secdPlanId, { boxid: 'loadExp', title: '载入公司经验', contentType: 'ajax', modal: true, submit_BtnName: '确定导入', cls: 'shadow-container', width: 450,
        onLoad: function (o) {
            //o.fbox.center();
        ...
        submit_cb: function (o) {
            ...
            if (confirm("载入模板将删除已有任务及其数据,确定载入吗？")) {
                self.grid.mask("载入中...");
                var _str = '?OffSetYear=' + setoffset_year.val() + "&OffSetMonth=" + setoffset_month.val() + "&OffSetDay=" + setoffset_day.val() + "&name=" + escape(insertPlanName.val());
