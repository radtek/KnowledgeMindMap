...
var commApi = {
    ...
    'taskStatImg': { '2': 'ico_grey.png', '3': 'ico_yellow.png', '4': 'ico_green.png', '-1': 'ico_orange.png' }, // 任务状态图标
    "tpl": ['<tr id="tr{taskId}" cp="1" isEdit="{isEdit}" noDelete="{noDelete}" nodeLevel="{nodeLevel}">',
		'<td field="torder" class="datagrid-td-rownumber" title="{nameTitle}"><div class="datagrid-cell-rownumber" style="color:{nameColor}">{rowindex}</div></td>',
        ...
        '<td field="factEdDate"{clre} style="text-align:center" width="120">{factEdDate}</td>',
...
function a2DataGrid(config) {
    ...
    this.init = function () {
        ...
        this.colorPk.click(function () {
            ...
            if ($("tr[id=tr" + _tid + "]").attr("isEdit") != "true") {
                return false;
            }
		...
		.dblclick(function () {
		    ...
		    if ($("tr[id=tr" + tid + "]").attr("isEdit") == "" && curField != "nodeProfId") {
		        return false;
		    }
		    //		    if (!self.hasRight) {
		    ...
		    if ($("tr[id=tr" + tid + "]").attr("isEdit") != "true") {
		        return false;
		    }
		    //		    if (!self.hasRight) {
		    //		        if (ownerUserId != self._userId) {
        ...
        this.grid.find("input.asyncImexp").click(function () {
            self.asyncImexp();
        });
...
a2DataGrid.prototype.updateTaskFlow = function () {
    var self = this, tid = self.curCell.tid, nodeTypeId, relationType;
    if (!confirm("更新将影响以该以计划为模板的计划任务流程设置，是否继续？"))
        return false;
    $.ajax({
        url: "/Home/CascadeUpdateCopyTaskFlow",
        type: 'post',
        data: { taskId: tid },
        dataType: 'json',
        error: function () {
...
a2DataGrid.prototype.asyncImexp = function () {//同步公司经验节点
   
    var secdPlanId = this.secdPlanId;
   
    if (!confirm("更新将影响以该以计划为模板的计划任务名称与新增节点，是否继续？"))
        return false;
    $.ajax({
        url: "/Home/AsyncExpPlanTask",
        type: 'post',
        data: { secdPlanId: secdPlanId },
        dataType: 'json',
        error: function () {
            $.tmsg("m_jfw", "未知错误，请联系服务器管理员，或者刷新页面重试", { infotype: 2 });
        },
...
a2DataGrid.prototype.updateTaskPhaseForm = function () {
    var self = this, tid = self.curCell.tid, nodeTypeId, relationType;
    if (!confirm("更新将影响以该任务为模板的计划任务表单关联，是否更新？"))
        return false;
    $.ajax({
        url: "/Home/CascadeUpdateCopyTaskPhaseForm",
        type: 'post',
        data: { taskId: tid },
        dataType: 'json',
        error: function () {
...
a2DataGrid.prototype.updateTaskFileRequest = function () {
    var self = this, tid = self.curCell.tid, nodeTypeId, relationType;
    if (!confirm("更新将影响以该任务为模板的计划任务流程文档与说明关联，是否更新？"))
        return false;
    $.ajax({
        url: "/Home/CascadeUpdateCopyTaskFileRequest",
        type: 'post',
        data: { taskId: tid },
        dataType: 'json',
        error: function () {
...
a2DataGrid.prototype.initEv = function () {
    ...
    this.dt.find("tr[isEdit]").unbind(".wkgrid").bind("mousedown.wkgrid", function (ev) {
        ...
        if ($("tr[id=tr" + ccell.tid + "]").attr("isEdit") != "true") {
            return false;
        }
        self.menu(e);
...
a2DataGrid.prototype.menu = function (e) { // menu
    ...
    _menu.find(".menu-cancelLink").unbind("click").bind("click", function () {
        deleteTaskRels(cell.tid); _menu.hide();
    })

    _menu.find(".menu-updateTaskFlow").unbind("click").bind("click", function () {
        self.updateTaskFlow.call(self); _menu.hide();
    });

    _menu.find(".menu-updateTaskPhaseForm").unbind("click").bind("click", function () {

        self.updateTaskPhaseForm.call(self); _menu.hide();
    });



    _menu.find(".menu-flowFile").unbind("click").bind("click", function () {
        setTaskFlowFile(cell.tid);
    });

    _menu.find(".menu-flowRemark").unbind("click").bind("click", function () {
        setTaskFlowRemark(cell.tid);
    });

    _menu.find(".menu-updateTaskFileRequest").unbind("click").bind("click", function () {

        self.updateTaskFileRequest.call(self); _menu.hide();
    });
...
//        }
    ...
    $.ajax({
        ...
        data: {
            ...
            ps: -1,
            cu: 1,
            qu: "db.System_Professional.distinct('_id',{'isPlanUse':'1'})",
            of:"order"
        },
        dataType: 'json',
        error: function () {
...
a2DataGrid.prototype.showEditor = function (l, t, w, h) { // 编辑单元格
    ...
    if (_col.editor) {
        ...
        if (_col.field == 'name' && relConDiagId > 0 && isPlanTemplate != true) {
...
a2DataGrid.prototype.importExp = function () { // 导入经验
    ...
    box(commApi.importExp + secdPlanId, { boxid: 'loadExp', title: '载入公司经验', contentType: 'ajax', modal: true, submit_BtnName: '确定导入', cls: 'shadow-container', width: 800,
        onLoad: function (o) {
            //o.fbox.center();
