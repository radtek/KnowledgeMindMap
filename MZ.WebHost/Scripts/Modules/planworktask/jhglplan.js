...
var a3plan = (function () {
    var urls = {
        ...
        savePlanInfo: '/Home/JHGL_SavePlanInfo',
        changestatus: '/Home/JHGL_ChangePlanStatus',
        hasCompleted: '/Home/JHGL_HasIncompleteTask'
    };
    return {
        ...
        changeStatus: function (obj, xd) {
            if (typeof (isManager) != "undefined" && parseInt(isManager) != 1) {
                $.tmsg("m_cgplan", "不是计划操作人无法操作", { infotype: 2 });
                return false;
            }
            var secdPlanId = $(obj).attr("data-planid"), onowstat = $("#nowstats" + secdPlanId), nst = onowstat.attr("nst"), newstat = $(obj).attr("newstat"), tip = "", pageId = onowstat.attr("pageId");
            ...
            $.ajax({
                url: urls.hasCompleted + "?_t=" + Math.random(), data: { planId: secdPlanId, status: newstat, pageId: pageId },
                success: function (data) {
                    data = data.Data;
                    if (data.success) {
                                ...
                                $("#planChangeForm1").validate({
                                    submitHandler: function (form) {
                                        ...
                                        $.ajax({
                                            ...
                                            data: dt + "&planId=" + secdPlanId + "&status=" + newstat + "&pageId=" + pageId,
                                            success: function (data) {
                                                o.fbox.unmask();
                                                if (data.Success) {
