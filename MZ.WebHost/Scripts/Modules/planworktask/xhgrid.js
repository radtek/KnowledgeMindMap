...
var commApi = {
    ...
    "setTaskPush": '/Home/SavePostInfo', // 设置配置清单
    ...
    'taskStatImg': { '-1': 'ico_orange.png', '2': 'ico_grey.png', '3': 'ico_yellow.png', '4': 'ico_green.png' }, // 任务状态图标
...
function a2DataGrid(config) {
    ...
    this.init = function () {
        ...
        this.canMap = this.grid.attr("canmap") || '1';//是否可以编辑地铁图节点
        this.canMap = this.canMap == '1' ? true : false;
		...
		.dblclick(function () {
		    ...
		    var cellindex = cell.colindex, _col = self.columns[cellindex];
		    if (!self.hasRight) {
		    ...
		    //20140928 关闭地铁图节点的编辑
		    if (_col.field == "pointName" && !self.canMap) {
		        return false;
		    }
		    self.initEditor();
...
a2DataGrid.prototype.calcrs_ = function (dt, lvl) {
    ...
    if (dt['name'] != "请填写任务名称") {
        ...
        _url = _isexpLibId ? '<a href="javascript:;">' : '<a href="' + commApi.taskUrl + _tid + '?isPlan=1" target="_blank" class="' + _cls + '">';
        ...
        if (dt['hasPush'] == 1) _exemption += '<img src="/content/images/icon/ico_send.png" title="成果推送" name="hasPush" style="vertical-align:middle;" />';
    ...
    if (dt.status!=-1 && dt.status <= 2) {
        ...
        dt["statusName"] = '<img title="' + dt["statusName"] + '" src="' + commApi.imgUrl + commApi.taskStatImg[dt.status] + '" alt="' + dt["statusName"] + '" />';
    //}
    padleft = 16 * lvl;
    dt['padleft'] = padleft;
    dt['nwidth'] = 312 - padleft - 18;
...
a2DataGrid.prototype.setISI = function () { // 是否首页显示
    ...
    var tpl = '<div style="width:360px;height:100px;"><ul style="display:block;margin:8px"></ul></div>', self = this, tid = self.curCell.tid,
		dt = self.getTaskFieldData(tid, "remark") + "", itc = "", itcRemark = "";
    box(tpl, { boxid: 'ISI', title: '任务设置显示(打钩表示设置成功)', cls: 'shadow-container', modal: true, width: 400,
        onOpen: function (o) {
            getISI(o);
        },
        submit_cb: function (o) {
            var lId, lrId = 0, lrTime = 0; arr = {};
            if ($("input[name=needUpload]", o.db).is(":checked")) {
                lrId = 1;
            } else {
                lrId = 0;
            }
            if ($("input[name=needCheckTime]", o.db).is(":checked")) {
                lrTime = 1;
            } else {
                lrTime = 0;
            }
            arr.needUpload = lrId;
            arr.needCheckTime = lrTime;
    ...
    function getISI(o) {
        ...
        $.post(commApi.projlinkData,
        {
            ...
            $(data).each(function (i, n) {
                //isLink 联动节点
                if (data[i].needUpload == "1") { itc = "checked=checked"; } else { itc = ""; } //阶段性成果Id
                if (data[i].needCheckTime == "1") { itc2 = "checked=checked"; } else { itc2 = ""; } //阶段性成果Id
                htmlLi += '<li  style="display:block;padding:4px"><input type="checkbox" ' + itc + ' name="needUpload" >是否需要上传交付物</li>';
                htmlLi += '<li  style="display:block;padding:4px"><input type="checkbox" ' + itc2 + ' name="needCheckTime" >是否需要设置开始和结束时间</li>';
            });
...
a2DataGrid.prototype.delTasks = function () { // 删除任务
    var tid, self = this, ids = "", f = [], relConDiagId = 0;
    for (tid in self.selRows) {
...
a2DataGrid.prototype.initEv = function () {
    ...
    this.dt.unbind(".wkgrid").bind("mousedown.wkgrid", function (ev) {
        ...
        if (_col.field == "pointName" && !self.canMap) {//关闭地铁图节点编辑
            return false;
        }
...
a2DataGrid.prototype.menu = function (e) { // menu
    ...
    _menu.find(".menu-jfw").unbind("click").bind("click", function () {
        ...
        if (typeof upLoadJfw != 'undefined' && $.isFunction(upLoadJfw)) {
            upLoadJfw(_ids);
        }
        _menu.hide();
    });
    ...
    _menu.find(".menu-hasPush").unbind("click").bind("click", function () {
        var hasPushFlag = self.getTaskFieldData(cell.tid, 'hasPush');
        if (hasPushFlag ==0) {
           self.setTaskPush.call(self, cell.tid, 1);
        } else {
            self.setTaskPush.call(self, cell.tid, 0);
        }
        _menu.hide();
    });
...
function lvlFormatter(value) {
    ...
    var tempLevel = [{ lvlid: '-1', lvlname: '请选择' }, { lvlid: '1', lvlname: '一级决策' }, { lvlid: '2', lvlname: '二级计划' }, { lvlid: '3', lvlname: '二级评审' }, { lvlid: '4', lvlname: '三级计划' }, { lvlid: '5', lvlname: '三级评审'}]
    for (; i < tempLevel.length; i++) {
        if (value == tempLevel[i].lvlid) isck = true;
        str += '<option value="' + tempLevel[i].lvlid + '"' + (isck ? " selected" : "") + '>' + tempLevel[i].lvlname + '</option>';
        isck = false;
    }
    return str;
}
...
a2DataGrid.prototype.saveTask = function (taskId, options) { // ajax task
    ...
    postDt['departRemark'] = Dt.departRemark;
    postDt['tbName'] = 'XH_DesignManage_Task';
    postDt['queryStr'] = "db.XH_DesignManage_Task.distinct('_id',{'taskId':'" + taskId + "'});";

    if (typeof options != "undefined") {
        postDt['needUpload'] = options.needUpload;
        postDt['needCheckTime'] = options.needCheckTime;
    }
...
a2DataGrid.prototype.setTaskPush = function (taskId, hasPush) {
    var me = this;
   
    $.ajax({
        type: 'POST', url: commApi.setTaskPush,
        data: { tbName: "XH_DesignManage_Task", "queryStr": "db.XH_DesignManage_Task.distinct('_id',{'taskId':'" + taskId + "'})", "hasPush": hasPush },
        success: function (rs) {
            if (!rs.Success) {
                $.tmsg("saveTask", rs.Message, { infotype: 2 });
            } else {
                var _tr = $("#tr" + taskId),
                    f = _tr.find("div:last"), hasc = me.getTaskFieldData(taskId, 'hasPush');
                var taskData = me.getTaskData(taskId);
                if (hasPush == 1) {
                    f.append('<img src="/content/images/icon/ico_send.png" title="成果推送" style="vertical-align:middle;" name="hasPush" />');
                    me.updateFieldData(taskId, 'hasPush',1);
                } else if (hasPush == 0) {
                    f.find('img[name="hasPush"]').remove();
                    me.updateFieldData(taskId, 'hasPush', 0);
                }
...
a2DataGrid.prototype.updateView1 = function (taskId, c, data) { // 刷新视图1
    ...
    if (field == 'name') {
        ...
        var _url = _isexpLibId ? '<a href="javascript:;">' : '<a href="' + commApi.taskUrl + taskId + '?isPlan=1" target="_blank">', exem = '';
        f = f.find("div:last");
...
a2DataGrid.prototype.calcFormula = function (formula, fRef, expression) { // S37 [S32,G32] S32+G32-5
    ...
    for (; i < fRef.length; ++i) {
        ...
        fRefData = this.getTaskFieldData(rr[2], ccodeToField[rr[1].toUpperCase()]).toString().replace("-", "/");
...
a2DataGrid.prototype.importExp = function () { // 导入经验
    var expData = [], _secdPlanId = -1, self = this, secdPlanId = self.secdPlanId, setoffset_year, setoffset_month, setoffset_day, isIgnorePerson, isIgnoreDate, isIgnorePassed, insertPlanName, managerId;
    box(commApi.importExp + secdPlanId, { boxid: 'loadExp', title: '载入公司经验', contentType: 'ajax', modal: true, submit_BtnName: '确定导入', cls: 'shadow-container', width: 800,
        onLoad: function (o) {
            ...
            insertPlanName = o.db.find("input[name='name']");
            managerId = o.db.find("input[name='managerId']");
        ...
        submit_cb: function (o) {
            ...
            var planName = $.trim(insertPlanName.val());
            if (planName == "undefined" || planName == "") {
                insertPlanName.focus();
                $.tmsg("cplan1", "请填写计划名称！");
                return false;
            }

            var manId = managerId.val();
            if (manId == -1 || manId == "undefined" || manId == 0) {
                managerId.focus();
                $.tmsg("cplan1", "请选择计划负责人！");
                return false;
            }
            ...
            if (confirm("载入模板将删除已有任务及其数据,确定载入吗？")) {
                self.grid.mask("载入中...");
                var _str = '?OffSetYear=' + setoffset_year.val() + "&OffSetMonth=" + setoffset_month.val() + "&OffSetDay=" + setoffset_day.val() + "&name=" + escape(planName) + "&managerId=" + manId;
