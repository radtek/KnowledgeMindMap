...
var commApi = {
    "getPlanData": "/Home/TaskListNew",
    //20150310 修改 QuickCreateTaskNew 为 QuickCreateProjDocTaskJH
    "createTasks": '/Home/QuickCreateProjDocTaskJH', // 创建任务
    //20150310 修改 DeleteTask 为 SyncDeleteProjDocTask
    "delTask": '/Home/SyncDeleteProjDocTask', // 删除任务
    "editFormula": '/Home/FormulaEdit', // 编辑公式
    "delFormula": '/Home/DeleteFormula', // 删除公式
    //20150310 修改 SaveTaskInfo 为 SaveProjDocTaskInfoJH
    "saveTask": '/Home/SaveProjDocTaskInfoJH', // 保存任务 原本为
    ...
    //20150310 修改 MovePostInfo 为 SyncMoveProjDocTask
    "moveTask": '/Home/SyncMoveProjDocTask', // 移动任务
    ...
    "docSettingUrl":'/DesignManage/TaskFileSetting/',//设置资料专业阶段
    'imgUrl': __globalDomain + '/Content/images/DesignManage/', // 任务状态图标目录
    'saveBgColor': '/Home/ChangeDateBgColor/', // 保存日期单元格底色
    'taskStatImg': { '2': '', '3': 'ico-stop.png', '4': 'ico-down.png' }, // 任务状态图标
    "tpl": ['<tr id="tr{taskId}" cp="1">',
       ...
       '<td field="docClass" style="text-align:center">{docClass}</td>',
        '<td style="display:none" field="period" style="text-align:center">{period}</td>',
        '<td style="display:none" field="startDate"{clrs}>{startDate}</td>',
		'<td style="display:none" field="endDate"{clre}>{endDate}</td>',
		'<td style="display:none" field="ownerName">{ownerName}</td>',
        '<td style="display:none" field="levelId">{levelName}</td>',
        '<td style="display:none" field="nodeTypeId">{nodeName}</td>',
        '<td style="display:none" field="hasApproval" style="text-align:center">{SetApproval}</td>',
        '<td style="display:none" field="pointName" swid="{strTextPId}" pointId="{strPointId}">{pointName}</td>',
		'<td style="display:none" field="status" style="text-align:center" class="norbor">{statusName}</td>',
		'</tr>'],
    "tplv": ['<tr cp="1">', '<td><div class="tb_con">{rowindex}</div></td>', '<td><div class="tb_con"><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" src="' + __globalDomain + '/Content/images/DesignManage/ico-Expand.gif"></div><div style="float:left">{name}</div></div></td>', '<td><div class="tb_con">{ownerName}</div></td><td><div class="tb_con">{startDate}</div></td><td><div class="tb_con">{endDate}</div></td>', '</tr>'],
     "tplv1": ['<tr cp="1">', '<td><div class="tb_con">{rowindex}</div></td>', '<td><div class="tb_con"><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" src="' + __globalDomain + '/Content/images/DesignManage/ico-Expand.gif"></div><div style="float:left">{name}</div></div></td>', '<td><div class="tb_con">{docClass}</div></td>', '</tr>']
}, saveTip = $(".savetip");
...
function a2DataGrid(config) {
    ...
    this.init = function () {
        ...
        //判断是否是项目资料目录
        this.isDoc = this.grid.attr("data-isDoc") || '1';
        this.isDoc = this.isDoc == '1' ? true : false;
...
a2DataGrid.prototype.calcrs_ = function (dt, lvl) {
    ...
    if (dt['name'] != "请填写目录名称") {
        ...
        _url = _isexpLibId ? '<a href="javascript:;">' : '<a href="javascript:void(0);" class="' + _cls + '">';
...
a2DataGrid.prototype.addDocType = function () { // 添加资料类别设置
    var tpl = '<textarea style="width:390px;height:150px;"></textarea>', self = this, tid = self.curCell.tid,
		dt = self.getTaskFieldData(tid, "docType") + "";
    box(commApi.docSettingUrl + "?taskId=" + tid + "&projId=" + self.projId + "&r=" + Math.random(), { boxid: 'docType', title: '资料专业阶段设置', cls: 'shadow-container', contentType: 'ajax', modal: true, no_cancel: true, width: 800,
        onLoad: function (o) {
            o.fbox.find("#confirmAdd").click(function () {
                if ($("#confirmAdd").attr("isCanAdd") == "0") { //不可点击的情况
                    return false;
                }
                var html = ' <tr id="tr{docPropertyId}"><td align="center" style="line-height: 18px; padding: 5px 3px;" width="400"><font class="fontblue">'
                var groundName = $("#sysProfId").find("option:selected").text() + "-" + $("#stageId").find("option:selected").text() + "-" + $("#fileCatId").find("option:selected").text();
                html += groundName;
                html += '</font></td><td align="center" style="width:100px"><a class="yh-a_red" href="javascript:void(0);" tbname="TaskDocProtery" ';
                html += 'querystr="db.TaskDocProtery.distinct';
                html += "('_id',{'docPropertyId':'{docPropertyId}'})";
                html += '"';
                html += ' onclick="deleTaskDocType(this,{docPropertyId});"';
                html += '>删除</a></td>';
                var obj = o.fbox.find("#docproperty");
                var formdata = o.fbox.find("#docproperty").serialize();
                $.ajax({
                    url: "/Home/SyncSaveProjDocType",
                    type: 'post',
                    data: formdata,
                    dataType: 'json',
                    error: function () {
                    ...
                    success: function (data) {
                        if (data.Success == false) {
                            ...
                            html = html.replace("{docPropertyId}", data.htInfo.docPropertyId);
                            html = html.replace("{docPropertyId}", data.htInfo.docPropertyId);
                            html = html.replace("{docPropertyId}", data.htInfo.docPropertyId);
                            //html.docPropetyId = data.htInfo.docPropertyId
                            if ($('#groudList tbody').length > 0) {
                                $('#groudList tbody').append(html);
                            } else {
                                $("#groudList").append(html);
                            }
                            $("#confirmAdd").attr("disabled", "disabled");
                        }
                    }
                });
                if ($("#confirmAdd").attr("isCanAdd") == "1") {
                    $("#confirmAdd").addClass("p-btn_gray");
                    $("#confirmAdd").attr("isCanAdd", "0");
                }
            });
        },
        submit_cb: function (o) {
            var newTr = $("#groudList").find("tr");
            var newGroud = "";
            if (typeof newTr != "undefined") { newGroud = $(newTr).first().find("td").first().find("font").text(); }
            var newtxt = o.db.find("textarea").val();
            if (dt != newtxt) {
                _td = document.getElementById('tr' + tid);
                _td = _td.childNodes;
                if (_td[5]) $(_td[2]).html(newGroud);
                //self.rows[self.rowsindex[tid].index]['docClass'] = newtxt;
                self.saveTask(tid);
            }
        }
    });
...
//同步删除模板类型
function deleTaskDocType(obj,id) {
    var postData = {
        tbName: "TaskDocProtery",
        queryStr: "db.TaskDocProtery.distinct('_id',{'docPropertyId':'" + id + "'})"
    };
    if (!confirm("确定要删除当前类别吗?")) {
        return false;
    }
    $.post("/home/SyncDelProjDocType", postData, function () {
        $.tmsg("", "删除成功", { infotype: 1, time_out: 700 });
        $(obj).closest("tr").remove();
        if ($("#groudList tr").length <= 0) { //如果没有文档类型则可添加
            $("#confirmAdd").removeClass("p-btn_gray");
            $("#confirmAdd").attr("isCanAdd", "1");
        }
    });
}
...
a2DataGrid.prototype.setISI = function () { // 是否首页显示
    ...
    box(tpl, { boxid: 'ISI', title: '任务首页显示(打钩表示设置成功)', cls: 'shadow-container', modal: true, width: 400,
        onOpen: function (o) {
            getISI(o);
        },
        submit_cb: function (o) {
            var lId, lrId = 0, arr = {};
            if ($("input[name=isShowIndex]", o.db).attr("checked") == "checked") {
                lrId = 1;
            } else {
                lrId = 0;
            }
        ...
        $.post(commApi.projlinkData,
                    {
                        ...
                        $(data).each(function (i, n) {
                            //isLink 联动节点
                            if (data[i].isShowIndex == "1") itc = "checked=checked"; //阶段性成果Id
                            htmlLi = '<li  style="display:block;padding:4px"><input type="checkbox" ' + itc + ' name="isShowIndex">设置为首页提醒任务： <strong>' + data[i].name + '</strong><li>';
                        });
...
a2DataGrid.prototype.delTasks = function () { // 删除任务
    ...
    var delTip = self.isDoc ? "确定删除选中的目录吗？" : "确定删除选中的任务吗？";
    if (confirm(delTip + exMsg)) {
        if (f.length == 0) {
            return;
        }
        this.grid.mask("数据处理中，请稍候...");
...
a2DataGrid.prototype.menu = function (e) { // menu
    ...
    //资料专业阶段设置
    _menu.find(".menu-docType").unbind("click").bind("click", function () {
        if (self.canDel || self.hasRight) {
            self.addDocType.call(self, true, 1);
        } else {
            $.tmsg("power", "你没有权限进行此操作");
        }
        _menu.hide();
...
a2DataGrid.prototype.updateView1 = function (taskId, c, data) { // 刷新视图1
    ...
    if (field == 'name') {
        ...
        var _url = _isexpLibId ? '<a href="javascript:;">' : '<a href="javascript:;">', exem = '';
        f = f.find("div:last");
...
a2DataGrid.prototype.importExp = function () { // 导入经验
    ...
    box(commApi.importExp + secdPlanId + "?rr=" + Math.random(), { boxid: 'loadExp', title: '载入目录模板', contentType: 'ajax', modal: true, submit_BtnName: '确定导入', cls: 'shadow-container', width: 800,
        onLoad: function (o) {
            //o.fbox.center();
                ...
                $(this).click(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            var cc = [], tpl = commApi.tplv1.join(""), ct = 0;
                            function rd(pid, lv) {
                                var dt, i = 0, _tid, _pid, tmp;
                                for (; dt = data[i]; ++i) {
                            ...
                            rd(0, 0);
                            cc = cc.join('');
                            if (cc == "") cc = "<tr><td colspan='3'>暂无内容</td></tr>";
            ...
            if (typeof o.db.find("li[secdPlanId]").first().attr("secdPlanId") != undefined) {
                o.db.find("li[secdPlanId]").first().click();
            }
        ...
        submit_cb: function (o) {
            ...
            if (confirm("载入模板将删除已有任务及其数据,确定载入吗？")) {
                ...
                $.ajax({
                    ...
                    complete: function (o) { self.grid.unmask(); },
                    success: function (data) {
                        if (data.Success) {
