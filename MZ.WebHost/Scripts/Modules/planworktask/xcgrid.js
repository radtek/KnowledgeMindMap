...
var commApi = {
    ...
    "createTasks": '/Home/QuickCreateTaskNew', // 创建任务
    ...
    "docSettingUrl":'/ProjectManage/TaskFileSetting/',//设置资料专业阶段
    ...
    "tpl": ['<tr id="tr{taskId}" cp="1">',
       ...
       '<td field="remark" style="text-align:center">{remark}</td>',
    ...
    box(commApi.docSettingUrl + "?taskId=" + tid + "&projId=" + self.projId + "&r=" + Math.random(), { boxid: 'docType', title: '资料专业阶段设置', cls: 'shadow-container', contentType: 'ajax', modal: true, no_cancel:true,width: 800,
        onLoad: function (o) {
            ...
            o.fbox.find("#confirmAdd").click(function () {
                ...
                var groundName = $("#stageId").find("option:selected").text() + "-" + $("#sysProfId").find("option:selected").text() + "-" + $("#fileCatId").find("option:selected").text();
                html += groundName;
                html += '</font></td><td style="width:100px"><a class="delete" href="javascript:void(0);" tbname="TaskDocProtery" ';
                html += 'querystr="db.TaskDocProtery.distinct';
                html += "('_id',{'docPropertyId':'{docPropertyId}'})";
                html += '"';
                html += ' onclick="CustomDeleteItem(this,delCall,{docPropertyId},';
                html = html + tid + ');">删除</a></td>';
                ...
                if ($("#confirmAdd").attr("isCanAdd") == "0") {
                    $("#confirmAdd").attr("disabled", true);
                }
                // zhtzMultiPlanManage.saveInfo(obj, tid, self.projId);
            });
            //o.db.find("textarea").val(dt);
