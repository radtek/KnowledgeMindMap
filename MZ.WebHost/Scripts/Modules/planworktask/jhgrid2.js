...
var commApi = {
    ...
    "saveAsExp": '/Home/SaveAsExpLibSSV2/', // 保存为经验
    ...
    'imgUrl': __globalDomain + '/Content/images/client/JH/', // 任务状态图标目录
    'saveBgColor': '/Home/ChangeDateBgColor/', // 保存日期单元格底色
    'taskStatImg': { '2': 'state_light/p-flag_7.png', '3': 'state_light/p-flag_8.png', '4': 'state_light/p-flag_0.png', '5': 'state_light/p-flag_1.png', '6': 'state_light/p-flag_9.png', '7': 'state_light/p-flag_10.png', '-1': 'state_light/p-flag_2.png' }, // 任务状态图标
    "tpl": ['<tr id="tr{taskId}" cp="1" isEdit="{isEdit}" noDelete="{noDelete}" nodeLevel="{nodeLevel}">',
		...
		//'<td field="ownerName" width="94">{ownerName}</td>',
        '<td field="nodeOrgId" width="95">{nodeOrgName}</td>',
        ...
        '<td field="relConDiagName" cdid="{relConDiagId}" width="150">{relConDiagName}</td>',
...
a2DataGrid.prototype.calcrs_ = function (dt, lvl) {
    ...
    if (dt.status <= 2 && dt.status > -1) {
        dt["statusName"] = '<img style="width:10px;height:10px;" src="' + commApi.imgUrl + commApi.taskStatImg[2] + '" alt="未开始" />';
    } else {
        dt["statusName"] = '<img style="width:10px;height:10px;" src="' + commApi.imgUrl + commApi.taskStatImg[dt.status] + '" alt="' + dt["statusName"] + '" />';
    }
    padleft = 16 * lvl;
    dt['padleft'] = padleft;
...
a2DataGrid.prototype.checkField = function () { // 保存前检查
    ...
    if (flag) {
        if (_editor == "Date") {
            ...
            result = _select.text();
            if (_col.field == 'levelId' || _col.field == 'nodeTypeId' || _col.field == 'nodeOrgId' || _col.field == 'nodeProfId') {
...
a2DataGrid.prototype.saveData = function (data) { // 保存 任务数据
    ...
    if (data.modified) {
        ...
        if (data.type == 'formula') { // 进入公式保存处理
            ...
            // 任务数据
            var dt = data.result;
            if (data.field == 'levelId' || data.field == 'nodeTypeId' || data.field == 'nodeOrgId' || data.field == 'nodeProfId') {
                dt = data.data.plvl;
...
a2DataGrid.prototype.saveTask = function (taskId) { // ajax task
    ...
    postDt['nodeOrgId'] = Dt.nodeOrgId; postDt['needSplit'] = "0"; postDt['splitUserId'] = "-1"; postDt['splitProfId'] = "0";
    postDt['nodeProfId'] = Dt.nodeProfId; postDt['needSplit'] = "0"; postDt['splitUserId'] = "-1"; postDt['splitProfId'] = "0";
    ...
    $.YH.box({
        ...
        ok: function () {
            ...
            if (confirm("载入模板将删除已有数据及其数据,确定载入吗？")) {
                ...
                $.ajax({
                    url: commApi.iExpSave + _str + "&isNoUpdatePlan=1", data: { copySecPlanId: _secdPlanId, sourceSecPlanId: self.secdPlanId }, type: 'POST',
