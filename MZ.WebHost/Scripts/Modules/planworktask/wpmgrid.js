...
var commApi = {
    ...
    "setLandMark": '/Home/SaveTaskInfo', // 保存里程碑
    ...
    "tpl": ['<tr id="tr{taskId}" cp="1">',
		...
		'<td field="name" class="{cls}" width="312"><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" isExpall="true" id="img{taskId}" src="' + __globalDomain + '/Content/images/DesignManage/ico14.jpg"/></div><div style="float:left;width:{nwidth}px">{name}{isLandMarkImgPath}{newTaskIcoImgPath}</div></td>',
        ...
        '<td field="taskResult" >{taskResult}</td>',
        '<td field="firstPartyDepartment" style="text-align:center">{firstPartyDepartment}</td>',
...
var hoverbg = '#d0e5f5', selbg = '#fcf1a9', copyDate = { planId: 0, taskId: 0, field: '', v: '', f: '' },
    ...
    landMarkicon = [__globalDomain + "/Content/images/icon/ico3.gif", __globalDomain + "/Content/images/icon/ico1.gif"];
    
     

function SetLandMark(obj) {
    var taskId = $(obj).attr("taskId");
    var isLandMark = $(obj).attr("isLandMark");
    if (isLandMark != "1") {
        $(obj).attr("isLandMark", "1")
        isLandMark = "1";
      //  $(obj).attr("src", landMarkicon[1])
    } else {
        isLandMark = "0";
        $(obj).attr("isLandMark", "0")
      //  $(obj).attr("src", landMarkicon[0])
    }

    $.ajax({
        type: 'POST', url: commApi.setLandMark,
        data: { tbName: "XH_DesignManage_Task", "queryStr": "db.XH_DesignManage_Task.distinct('_id',{'taskId':'" + taskId + "'})", "isLandMark": isLandMark, "taskId": taskId },
        success: function (rs) {
            if (!rs.Success) {
...
a2DataGrid.prototype.addRemark = function () { // 添加备注
    var tpl = '<textarea style="width:350px;height:100px; margin:10px 20px"></textarea>', self = this, tid = self.curCell.tid,
...
// 2015.3.31新增 更新任务节点时间  by 5220
a2DataGrid.prototype.updateTaskTime = function () {
    var self = this, tid = self.curCell.tid;
    if (!confirm("是否同步当前任务计划时间与工期？"))
        return false;
    $.ajax({
        url: "/Home/UpdateTaskTime",
        type: "post",
        data: { taskId: tid },
        dataType: "json",
        error: function () {
...
a2DataGrid.prototype.initEv = function () {
    ...
    this.dt.unbind(".wkgrid").bind("mousedown.wkgrid", function (ev) {
        ...
        if (_tgn == 'img' && $(tag).attr('isExpall') && tag.name != "cilist" && tag.parentNode.tagName.toLowerCase() != 'td') {
            self.ec(tag);
        }
        tag = getTd(tag);
        _idx = $(tag).index();
...
a2DataGrid.prototype.menu = function (e) { // menu
    ...
    // 更新节点时间
    _menu.find(".menu-updateTime").unbind("click").bind("click", function () {
        self.updateTaskTime.call(self); _menu.hide();
    });

    _menu.find(".menu-batchStart").unbind("click").bind("click", function (ev) {
        if (self.hasRight) {
            self.batchStart.call(self,ev);
        } else {
            $.tmsg("power", "你没有权限进行此操作");
        }
        _menu.hide();
    ...
    _menu.find(".menu-batchFinish").unbind("click").bind("click", function (ev) {
        if (self.hasRight) {
            self.batchFinish.call(self, ev);
        } else {
            $.tmsg("power", "你没有权限进行此操作");
        }
        _menu.hide();
        ...
        _menu.find(".menu-batchSetNew").unbind("click").bind("click", function (ev) {
        if (self.hasRight) {
            self.batchSetNew.call(self, ev);
        } else {
            $.tmsg("power", "你没有权限进行此操作");
        }
        _menu.hide();
    });
        _menu.find(".menu-batchCancelNew").unbind("click").bind("click", function (ev) {
        if (self.hasRight) {
            self.batchCancelNew.call(self, ev);
        } else {
            $.tmsg("power", "你没有权限进行此操作");
        }
        _menu.hide();
...
a2DataGrid.prototype.batchStart = function (ev) { // 开始选中的任务
    ...
    var boxHtml = "<div><table width='100%' class='m10'><tr><td width='60'>备注：</td><td><textarea id='txtRemark' style='width:300px;height:100px'></textarea></td></tr></table></div>";
    box(boxHtml, { boxid: 'startRemark', title: "批量开始任务", contentType: 'selector', width: 400, submit_BtnName: "确 定",
        submit_cb: function (o) {
            var value = o.fbox.find("textarea").val();
         
                $.ajax({
                    url: "/Home/BatchChangeTaskStatus",
                    type: "post",
                    data: { taskIds: ids.join(','), status: "3", remark: value },
                    dataType: "json",
                    error: function () {
                        $.tmsg("m_jfw", "未知错误，请联系服务器管理员，或者刷新页面重试", { infotype: 2 });
                    },
...
a2DataGrid.prototype.batchFinish = function (ev) { // 批量完成选中的任务
    var x = YH.dom.getMouseX(ev), y = YH.dom.getMouseY(ev),
        ids = [], rows = this.dt[0].rows, self = this, tid;
    for (tid in self.selRows) {
        ids.push(tid);
    ...
    var boxHtml = "<div><table width='100%' class='m10'><tr><td width='60'>备注：</td><td><textarea id='txtRemark' style='width:300px;height:100px'></textarea></td></tr></table></div>";
    box(boxHtml, { boxid: 'finishRemark', title: "批量完成任务", contentType: 'selector', width: 400, submit_BtnName: "确 定",
        submit_cb: function (o) {
            var value = o.fbox.find("textarea").val();
                ...
                $.ajax({
                    url: "/Home/BatchChangeTaskStatus",
                    type: "post",
                    data: { taskIds: ids.join(','), status: "4", remark: value },
                    dataType: "json",
                    error: function () {
                        $.tmsg("m_jfw", "未知错误，请联系服务器管理员，或者刷新页面重试", { infotype: 2 });
                    },
...
a2DataGrid.prototype.batchSetNew = function (ev) { // 批量设置为新插入的任务
    var x = YH.dom.getMouseX(ev), y = YH.dom.getMouseY(ev),
        ids = [], rows = this.dt[0].rows, self = this, tid;
    for (tid in self.selRows) {
        ids.push(tid);
    ...
    var boxHtml = "<div><table width='100%' class='m10'><tr><td width='60'>备注：</td><td><textarea id='txtRemark' style='width:300px;height:100px'></textarea></td></tr></table></div>";
    box(boxHtml, { boxid: 'finishRemark', title: "批量设为新插入任务", contentType: 'selector', width: 400, submit_BtnName: "确 定",
        submit_cb: function (o) {
            var value = o.fbox.find("textarea").val();
             
                $.ajax({
                    url: "/Home/BatchSetNewTask",
                    type: "post",
                    data: { taskIds: ids.join(','),  remark: value ,isNewTask:"1"},
                    dataType: "json",
                    error: function () {
                        $.tmsg("m_jfw", "未知错误，请联系服务器管理员，或者刷新页面重试", { infotype: 2 });
                    },
    ...
    a2DataGrid.prototype.batchCancelNew = function (ev) { // 批量设置为新插入的任务
    var x = YH.dom.getMouseX(ev), y = YH.dom.getMouseY(ev),
        ids = [], rows = this.dt[0].rows, self = this, tid;
    for (tid in self.selRows) {
        ids.push(tid);
    ...
    var boxHtml = "<div><table width='100%' class='m10'><tr><td width='60'>备注：</td><td><textarea id='txtRemark' style='width:300px;height:100px'></textarea></td></tr></table></div>";
    box(boxHtml, { boxid: 'finishRemark', title: "批量重置为非新增任务", contentType: 'selector', width: 400, submit_BtnName: "确 定",
        submit_cb: function (o) {
            var value = o.fbox.find("textarea").val();
               $.ajax({
                    url: "/Home/BatchSetNewTask",
                    type: "post",
                    data: { taskIds: ids.join(','),  remark: value ,isNewTask:"0"},
                    dataType: "json",
                    error: function () {
                        $.tmsg("m_jfw", "未知错误，请联系服务器管理员，或者刷新页面重试", { infotype: 2 });
                    },
...
a2DataGrid.prototype.saveTask = function (taskId, options) { // ajax task
    ...
    postDt['curStartData'] = Dt.startDate;
    postDt['curEndData'] = Dt.endDate; 
    postDt['remark'] = Dt.remark;
    postDt['period'] = Dt.period;
    postDt['taskResult'] = Dt.taskResult;
    postDt['firstPartyDepartment'] = Dt.firstPartyDepartment;
    postDt['firstPartyDepartmentUserName'] = Dt.firstPartyDepartmentUserName;
