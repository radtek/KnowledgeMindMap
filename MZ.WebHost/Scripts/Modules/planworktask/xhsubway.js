...
function getRelTasks(planId, sid, obj, pointId) {
    ...
    $.ajax({
        ...
        success: function (rs) {
            ...
            for (; i < rs.length; ++i) {
                _ = rs[i];
                cc.push('<tr><td height=25 class="fontblue"></td><td class="fontblue" width="250"><a target="_blank" href="/DesignManage/ProjTaskInfo/' + _.id + '">' + _.name + '</a></td><td width="100">' + _.otherParam + '</td><td class="fontorange" width="50">' + _.value + '</td></tr>'); has = true;
            }
            cc.push("</table>");
            if (has) {
                tpane.html(cc.join(""));
