...
var SelTree = function (arg) {
    ...
    switch (val) {
        case "orgpost":
            ...
            $(_gdoc).find("#ifrm").html("").SelectTree({
...
function btnTreeSearchFunc() {
    ...
    window.ifrm.location.href = "/HumanResource/SearchTree?type=" + val + "&key=" + encodeURI(key);
    ...
    if ($("#selUser option:selected").length > 0) {
        if (isSignal) {
            if ($("#selUser option:selected").length > 1) {
                if (typeof showInfoBar === 'function') {
...
var bindASearchableInput = function (o, userIdObj, profIdObj, projectId, type, hasProfSelect, needsearch) {
    ...
    $(o).unbind("keyup").bind("keyup", function (ev) {
        addKeyboardControlSupport(o, userIdObj, profIdObj, projectId, type, hasProfSelect, ev);
    });
...
var bindASearchableInput_ZHHY = function (o, userIdObj, profIdObj,namesHidenObj, projectId, type, hasProfSelect, needsearch, defaultUserArr) {
    userIdObject = userIdObj;
    profIdObject = profIdObj;

    $(o).unbind("focus").bind("focus", function () {
        showTaskPeopleList_ZHHY(o, userIdObj, profIdObj,namesHidenObj, projectId, type, hasProfSelect, needsearch, defaultUserArr);
    });
    $(o).unbind("keyup").bind("keyup", function (ev) {
        addKeyboardControlSupport(o, userIdObj, profIdObj, projectId, type, hasProfSelect, ev);
    });
...
function addKeyboardControlSupport(o, userIdObj, profIdObj, projectId, type, hasProfSelect, ev) {
...
function confirmTheItemSelection(thisAObject, o, userIdObj, profIdObj, hasProfSelect) {
    hideAllTaskPeopleList(); //隐藏人员选择列表
...
function getSearchTaskPeopleList(o, userIdObj, profIdObj, hasProfSelect) {
    currentListIndex = 0;
    var urls = {
        getTaskPeople: '/Home/PlanRelatedPersonnelJHGL?r=' + Math.random(),  //取当前项目相关人员
...
function getTaskPeopleList(o, userIdObj, profIdObj, projectId, type) {
    ...
    $.getJSON(urls.getTaskPeople, { projId: projectId, type: type,planId:planId }, function (data) {
        data = data.Data;
...
//显示人员选择窗体，并初始化系统选人框
function showTaskPeopleList_ZHHY(o, userIdObj, profIdObj,namesHiddenObj, projectId, type, hasProfSelect, needsearch, defaultUserArr) {

    function initDefaultUsers(o) {
        var i = 0, _user;
        if (!(defaultUserArr instanceof Array) || defaultUserArr.length <= 0) {
            return;
        }
        for (; _user = defaultUserArr[i]; i++) {
            o.db.find("#selAdd").append("<option value='" + _user.userId + "'>" + _user.userName + "</option>");
        }
    };
    ...
    $("#selectUserForProf").click(function () {
        ...
        SelectUsers.init({
            ...
            callback: function (rs) {
                var id, name;
                var ids = [], names = [], nameFormatArr = [];
                for (var key in rs) {
                    id = key, name = rs[key];
                    ids.push(id);
                    names.push(name);
                    nameFormatArr.push(id + "|Y|" + name);
                }
                $(o).val(names.join(","));
                $(namesHiddenObj).val(nameFormatArr.join("|H|"));// 1|Y|张三|H|2|Y|王五
                $(userIdObj).val(ids.join(","));
            }
        });
    });
        ...
        if (data.Success) {
            ...
            $("ul.aui-list-section").find("a").each(function () {
                ...
                $(this).click(function () {
                    hideAllTaskPeopleList();
                    var userArr = [];
                    userArr.push({ userId: $(this).attr("id"), userName: $(this).attr("name") });
                    cAjax($(this).attr("id"), $(this).attr("sysProfId"), $(this).attr("name"), userArr);
                });
            });
            $("#selectUserForProf").click(function () {
                var userIdArr = [], userNameArr = [];
                hideAllTaskPeopleList();
                SelectUsers.init({
                    rType: "cb",
                    prt: "selUser",
                    ...
                    callback: function (rs) {
                        ...
                        for (var key in rs) {
                            userIdArr.push(key);
                            userNameArr.push(rs[key]);
                            userArr.push({ userId: key, userName: rs[key] });
                            // SelectProfWithPersonForOneClick(rs[key], key, cAjax);
                        }
                        cAjax(userIdArr.join(","), '', userNameArr.join(","), userArr);
                    }
                });
            });
