...
var contrast = {
    ...
    checkStatus: function () {
        var $addInvisibleTr = $();
        var $removeInvisibleTr = $();
        $('[trtype="item"]').each(function () {
            if (!$(this).find('select:visible').length) {
                $addInvisibleTr = $addInvisibleTr.add($(this).nextUntil('[trtype="item"]'));
            } else {
                $removeInvisibleTr = $removeInvisibleTr.add($(this).nextUntil('[trtype="item"]'));
            }
        });
        $addInvisibleTr.addClass('p-invisibleTr')
        $removeInvisibleTr.removeClass('p-invisibleTr');
...
function GetResult(obj, type) {
    ...
    var formatId=$("#p-j-format").val();
    var $selnums = $("select[selnum=" + type + "]");
    $selnums.each(function () {
        $(this).empty();
    });
    contrast.loadCount++;
    $.post("/Home/CPIMGetAllResultAllFieldList?lineId=" + lineId + "&formatId=" + formatId,
        function (data) {
            ...
            $selnums.each(function () {
                var $cur = $(this);
                ($cur.find("option").length > 0) ? $cur.show() : $cur.hide();

                // $(this).change();
                if ($cur.find("option").length == 1) {
...
function GetProjResult(obj, type) {
    ...
    var formatId = $("#p-j-format").val();
    var $selprojnums = $("select[selprojnum=" + type + "]");
    $selprojnums.each(function () {
        $(this).empty();
    });
