...
function changeModuleModule(obj, combid) {
    ...
    getModuleNum();
    var url = [
            "/Home/GetProductThemeLevelModuleRelationJson?",
            "themeId="+themeId,
            "&boolUse=" + boolUse,
            "&levelId=" + levelId
        ].join('');
...
$('#p-j-treecontent').delegate('.mt-name, .mt-title', 'contextmenu', function (e) {
    ...
    $('#YH-MM-J-menu').on('click', '#YH-MM-J-nodeEdit', function () {
        ...
        //        }
        var fnOk = function () {
            $('.YH-MM-J-dropable').undelegate('.YH-MM-J-targetBlock .YH-MM-node-con,.YH-MM-box-mainNode', 'click')
                                  .removeClass('YH-MM-J-dropable');
