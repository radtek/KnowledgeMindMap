...
(function ($) {
    ...
    function _getNowD(_2e) {
        var _2f = $.data(_2e, "calendar").options,
        _now = new Date();
        _2f.onSelect.call(_2e, _now);
    }
    function _7(_8) {
        var _a1c = $.data(_8, "calendar").options;
        $(_8).addClass("calendar").wrapInner("<div class=\"calendar-header\">" + "<div class=\"calendar-prevmonth\"></div>" + "<div class=\"calendar-nextmonth\"></div>" + "<div class=\"calendar-prevyear\"></div>" + "<div class=\"calendar-nextyear\"></div>" + "<div class=\"calendar-title\">" + "<span>Aprial 2010</span>" + "</div>" + "</div>" + "<div class=\"calendar-body\">" + "<div class=\"calendar-menu\">" + "<div class=\"calendar-menu-year-inner\">" + "<span class=\"calendar-menu-prev\"></span>" + "<span><input class=\"calendar-menu-year\" type=\"text\"></input></span>" + "<span class=\"calendar-menu-next\"></span>" + "</div>" + "<div class=\"calendar-menu-month-inner\">" + "</div>" + "</div>" + "</div>" + (_a1c.el ? "<div class=\"calendar-optbtns\"><input type=\"button\" value=\"清空\"> <input type=\"button\" value=\"今天\"> <input type=\"button\" value=\"关闭\"></div>" : ""));
        ...
        if (_a1c.el) {
            var _ginpts = $(_8).find(".calendar-optbtns").find("input");
            $(_ginpts.eq(0)).unbind("click").bind("click",
            function () {
                $(_a1c.el).val('').focus();
            });
            $(_ginpts.eq(1)).unbind("click").bind("click",
            function () {
                _getNowD(_8);
            });
            $(_ginpts.eq(2)).unbind("click").bind("click",
            function () {
                var _pp = $(_8).parent();
                if (_pp[0]) _pp.hide();
            });
