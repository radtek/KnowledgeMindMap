    ...
    YinHoo.pagination = function (jq, opts) {
        ...
        function PagCal(jq) { // jQuery.pagination
            ...
            return {
                ...
                getInterval: function (current_page) {
                    ...
                    var start = current_page > _half ? Math.max(Math.min((parseInt(current_page) - parseInt(_half)), upper_limit), 0) : 0;
                    var end = current_page > _half ? Math.min((parseInt(current_page) + parseInt(_half) + parseInt(_opt.display_pc % 2)), np) : Math.min(_opt.display_pc, np);

                    return { start: start, end: end };
                },
                createLink: function (pageid, current_page) {
                    var ps = parseInt(this.getPages()), strLink;
                    pageid = pageid < 0 ? 0 : (pageid < ps ? pageid : (ps - 1));
                    if (pageid == (parseInt(current_page) - 1)) {
                        strLink = $("<span class=\"current\" style='*white-space: nowrap;'>" + (pageid + 1) + "</span>");
                    } else {
                        strLink = $("<a style='*white-space: nowrap;'>" + (pageid + 1) + "</a>").attr("href", _opt.link_to.replace(/__id__/, pageid + 1));
                    }
                ...
                getLinks: function (current_page, eventHandler) {
                    ...
                    if (interval.end < ps && _opt.side_pc > 0) {
                        ...
                        begin = Math.max((parseInt(ps) - parseInt(_opt.side_pc)), interval.end);
        ...
        function render(jq) {
            ...
            if (_opt.showInput) {
                ...
                $("<td><input class=\"pagination-num\" type=\"text\" value=\"1\" size=\"2\" style=\"width:40px;\"></td>").appendTo(tr);
            ...
            jq.find("a[icon=pagination-prev]").unbind(".pagination").bind("click.pagination", function () {
                if (_opt.current_page > 1) {
                    SelectPage(jq, (parseInt(_opt.current_page) - 1));
                }
            });
