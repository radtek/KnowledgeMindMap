myFocus.pattern.extend({//*********************fscreen******************
	'mF_fscreen_tb':function(settings,$){
		...
		$thumbBg.css({ height: 50 });
		$thumbBox.css({width:thuBoxWidth,height:50,left:p.thumbBtnWidth});
		$thumbUl.css({width:thuWidth*n});
		$thumbList.each(function(){this.style.width=thuWidth+'px'});
		$thumbBox.find('img').each(function () { this.style.height = 30+"px" }); //(p.thumbHeight - 9 * 2) + 'px'; //8px margin+1px border
