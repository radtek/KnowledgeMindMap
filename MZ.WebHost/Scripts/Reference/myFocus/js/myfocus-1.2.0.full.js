...
(function () {
    var mF = {
        defConfig: {//全局默认设置
            ...
            path: '/Scripts/Reference/myFocus/js/pattern/', //pattern文件的路径
    ...
    var DOM = {
        ...
        addmF_pithy_tbList: function (obj, cla) {
            ...
            for (var j = 0; j < num; j++) {
                ...
                for (var i = 0; i < n; i++) {
                    img = this.$$('img', li[i])[0];
                    //                    var curhtml = '<div class="scroll_block select_bg">';
                    //                    curhtml += ' <div class="S_pic"><img src="';
                    //                    curhtml += (img.getAttribute("thumb") || img.src)
                    //                    curhtml += '" /></div>';
                    //                    curhtml += '   <div class="scroll_block_info">';
                    //                    curhtml += '    <div class="name"><a href="';
                    //                    curhtml += img.getAttribute("linkUrl");
                    //                    curhtml += ' "></a></div>';

                    //                    curhtml += '  <div class="from">';
                    //                    curhtml += img.getAttribute("text")
                    //                    curhtml += '</div>';
                    //                    curhtml += '     </div>';
                    //                    curhtml += '   </div>';
                    ...
                    s.push('<li>' + curhtml + '<span></span></li>')
	...
	Init = {
	    ...
	    initCSS: function (p) {
	        ...
	        if (p.css) css.push('.' + p.S + ' *{margin:0;padding:0;border:0;list-style:none;}.' + p.S + '{position:relative;width:' + w + 'px;height:' + h + 'px;overflow:hidden;font:12px/1.5 Verdana;text-align:left;background:#fff;visibility:visible!important;}.' + p.S + ' .loading{position:absolute;z-index:9999;width:100%;height:100%;color:#666;text-align:center;padding-top:' + 0.3 * h + 'px;background:#fff url(/Content/images/icon/loading.gif) center ' + 0.4 * h + 'px no-repeat;}.' + p.S + ' .pic{position:relative;width:' + w + 'px;height:' + h + 'px;overflow:hidden;}.' + p.S + ' .txt li,.' + p.S + ' .txt li span,.' + p.S + ' .txt-bg{width:' + w + 'px;height:' + p.txtHeight + 'px!important;line-height:' + p.txtHeight + 'px!important;overflow:hidden;}.' + p.S + ' .txt li p a{display:inline;}');
    ...
    else window.console && console.log('"myFocus" has been Defined, Please use "myFocus__AGENT__".');
    //支持JQ
