...
<% 
    ...
    var isSuperUser = Yinhe.ProcessingCenter.Permissions.AuthManage.UserType == Yinhe.ProcessingCenter.Common.UserTypeEnum.DebugUser;//user的type为1
                                        ...
                                        <td>
                                            来自：<%=user.Text("name")%>&nbsp;&nbsp; <span style="color: #A49B9C">
                                                ...
                                                <%if (revert.Text("createUserId") == userId.ToString())
                                                  {%>
                                                  <a hidefocus="true" href="javascript:void(0)" class="edit" onclick="showCommentEdit('RevertEdit','/Evaluation/RevertEdit/?id=<%=revert.Text("revertId")%>&coid=<%=comObjId %>&r=<%=DateTime.Now.Ticks %>');">编辑</a>
                                                  <%}%>
                                                  <%if (revert.Text("createUserId") == userId.ToString()){%>
                                                  <a hidefocus="true" href="javascript:void(0)" class="delete" onclick="CustomDeleteItem(this,reload);" tbname="Evaluation_CommentReply" querystr="db.Evaluation_CommentReply.distinct('_id',{'revertId':'<%=revert.Int("revertId") %>'})">删除</a>
                                                  <%}%>
                                            </span>
                                        </td>
