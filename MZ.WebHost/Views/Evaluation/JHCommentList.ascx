...
<% 
    ...
    // var comments = dataOp.FindAllByKeyVal("EvaluationComment", "commentObjectId", comObjId.ToString()).ToList();
    //var comments = dataOp.FindAllByQueryStr("EvaluationComment", string.Format("commentObjectId={0}&objectId={1}", comObjId, objId)).ToList();
    var comments = dataOp.FindAllByQueryStr("EvaluationComment", string.Format("tableName={0}&keyName={1}&keyValue={2}", tableName, keyName, keyValue)).ToList();
    var userId = dataOp.GetCurrentUserId();//获取用户Id

    var isSuperUser = AuthManage.UserType == Yinhe.ProcessingCenter.Common.UserTypeEnum.DebugUser;//user的type为1
                ...
                <div style="margin: 0px 8px 8px 8px">
                    <table width="100%" style="margin-bottom: 10px">
                        <tr bgcolor="#f2f2f2" style="padding: 0px 5px">
                            ...
                            <td align="right">
                                <span style="cursor: pointer" onclick='$("#divCommentList").load("/Evaluation/JHRevertList?keyName=<%=keyName %>&keyValue=<%=keyValue %>&tableName=<%=tableName %>&comentId=<%=comment.Text("commentId") %>&r=" + Math.random());'>
                    ...
                    <div style="line-height: 16px; color: #424242; padding: 5px 8px 10px 8px">
                        ...
                        <a href="javascript:void(0)" class="blue" onclick='$("#divCommentList").load("/Evaluation/JHRevertList?keyName=<%=keyName %>&keyValue=<%=keyValue %>&tableName=<%=tableName %>&comentId=<%=comment.Text("commentId") %>&r=" + Math.random());'>
                            回复</a></div>
                        ...
                        <table width="100%">
                            ...
                            <tr>
                                <td align="right" colspan="2" height="25">
                                    <%-- lix新增判断20140612 --%>
                                    <% if (comment.Int("createUserId") == userId || isSuperUser)
                                       { %>
                                    <%--<a hidefocus="true" href="javascript:void(0)" class="edit" onclick="showCommentEdit('CommentEdit','/Evaluation/JHCommentEdit/?id=<%=comment.Text("commentId")%>&coid=<%=comObjId %>&r=<%=DateTime.Now.Ticks %>');">编辑</a>--%>
                                    <a hidefocus="true" href="javascript:void(0)" class="edit" onclick="showCommentEdit('CommentEdit','/Evaluation/JHCommentEdit/?id=<%=comment.Text("commentId")%>&r=<%=DateTime.Now.Ticks %>');">编辑</a>
    ...
    <div style="margin: 0px 8px 8px 8px">
        <table width="100%">
            ...
            <tr>
                ...
                <td>
                    <a class="btn_01" hidefocus="true" href="javascript:;" style="cursor: pointer" onclick="addCommentContent();">
                        提交评价<span></span></a>
                </td>
            </tr>
        </table>
...
<script type="text/javascript">
    ...
    function Sort(type) {
        $("#divCommentList").load("/Evaluation/JHCommentList?tableName=<%=tableName %>&keyName=<%=keyName %>&keyValue=<%=keyValue %>&r=" + Math.random());
    }

    //添加任务书comment
    function addCommentContent() {
        if ($("#CommentDesc").val() == "") {
            MZ.msgbox.show("请填写评价内容", 5);
            return false;
        }
